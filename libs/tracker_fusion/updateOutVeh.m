...
%=================左右车道最近轨迹========================
global g_ego_status
minMid = 200; idxMsecond = uint8(255);
minYL = 200; minYLSecond = 200; idxL = uint8(255); idxLSecond = uint8(255);
minYR = 200; minYRSecond = 200; idxR = uint8(255); idxRSecond = uint8(255);
def_wid_half = 2;
isFillFilter = false(obstacle_const.max_obs_fuse,1);
fusedStateMatrix = false(obstacle_const.max_obs_fuse,1);
if tracks.idxCIPV ~= 255 && tracks.idxCIPV > 0
    obj_Num = 1;
else
    obj_Num = 0;
end
for i = 1:obstacle_const.max_obs_fuse
    if ~isFill(i) || i== tracks.idxCIPV
        continue;
    end
    ...
    if fusedState
        fusedStateMatrix(i) = true;
        obj_Num = obj_Num+1;
    end
    % 筛选中间车道第二近的目标
    if tracks.output(i).x>xl && tracks.output(i).x<xr && i ~= tracks.idxCIPV
        if tracks.output(i).y < minMid
            minMid = tracks.output(i).y;
            idxMsecond = uint8(i);
        end
    end
    ...
    if tracks.output(i).x<xl && tracks.output(i).x>(xl-lane_const.cfg.def_wid_half*2)
        if tracks.output(i).y<minYL
            minYLSecond = minYL;
            idxLSecond = idxL;
            minYL =  tracks.output(i).y;
            idxL = uint8(i);
        elseif tracks.output(i).y<minYLSecond
            minYLSecond = tracks.output(i).y;
            idxLSecond = uint8(i);
        end
    end
    ...
    if tracks.output(i).x>xr && tracks.output(i).x<(xr+lane_const.cfg.def_wid_half*2)
        if tracks.output(i).y<minYR
            minYRSecond = minYR;
            idxRSecond = idxR;
            minYR =  tracks.output(i).y;
            idxR = uint8(i);
        elseif tracks.output(i).y<minYRSecond
            minYRSecond = tracks.output(i).y;
            idxRSecond = uint8(i);
        end
    end
...
if idxR ~= 255 && tracks.output(idxR).y <= 20 && (tracks.output(idxR).velo_y + g_ego_status.speed/3.6 >=3)
    if fusedStateMatrix(idxR) == false
        obj_Num = obj_Num+1;
    end
    fusedStateMatrix(idxR) = true;
end
if idxL ~= 255 && tracks.output(idxL).y <= 20 && (tracks.output(idxL).velo_y + g_ego_status.speed/3.6 >=3)
    if fusedStateMatrix(idxL) == false
        obj_Num = obj_Num+1;
    end
    fusedStateMatrix(idxL) = true;
end

% 如果有效目标小于6，则全部输出有效目标
% 如果有效目标大于6，则进行目标筛选
% 筛选目标，左边2个目标，右边2个目标,中间第二远的目标
if obj_Num <= 6
    isFillFilter = fusedStateMatrix;
else
    if idxR ~= 255 && fusedStateMatrix(idxR) == true
        isFillFilter(idxR) = true;
    end

    if idxL ~= 255 && fusedStateMatrix(idxL) == true
        isFillFilter(idxL) = true;
    end

    if idxRSecond ~= 255 && fusedStateMatrix(idxRSecond) == true
        isFillFilter(idxRSecond) = true;
    end

    if idxLSecond ~= 255 && fusedStateMatrix(idxLSecond) == true
        isFillFilter(idxLSecond) = true;
    end

    if idxMsecond ~= 255 && fusedStateMatrix(idxMsecond) == true
        isFillFilter(idxMsecond) = true;
    end
end
end
...
for i = 1:obstacle_const.max_obs_fuse
    ...
    BlindSpotTarget = ((tracks.output(i).y <=3 && tracks.output(i).y >=-9 && tracks.output(i).x >=-4.2 && tracks.output(i).x <=4.2) &&...
        (tracks.output(i).velo_y + g_ego_status.speed/3.6 >=2)&& (abs(g_ego_status.yawrate)<30 || abs(g_ego_status.SW_angle)<5) &&...
        (tracks.output(i).x <=-1.2 || tracks.output(i).x >=1.2 || tracks.output(i).y <=-6 || tracks.output(i).y >=0)); % 判断能否输出

     % 输出右转向盲区内角雷达目标
    RightTurnBlindSpotTarget = ((tracks.output(i).y <=3 && tracks.output(i).y >=-10.415 && tracks.output(i).x >=0.5 && tracks.output(i).x <=5) &&...
    ...
    idOverlap=uint8(0);
    for j=1:i-1
        if (tracks.output(i).det_src== det_src.left_forward && tracks.output(j).det_src== det_src.left_backward) ||...
           (tracks.output(i).det_src== det_src.left_backward && tracks.output(j).det_src== det_src.left_forward) ||...
           (tracks.output(i).det_src== det_src.right_forward && tracks.output(j).det_src== det_src.right_backward) ||...
           (tracks.output(i).det_src== det_src.right_backward && tracks.output(j).det_src== det_src.right_forward) 
