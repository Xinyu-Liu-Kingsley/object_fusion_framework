function plotResultsFusionMDCU(ax, frameDataSingleMDCU)
...
x_MDCU = zeros(1, obstacle_const.max_obs_camera);
y_MDCU = zeros(1, obstacle_const.max_obs_camera);
MDCUObjects = frameDataSingleMDCU.CameraFrame.CameraObjectList;
...
% %         plot(cameraObjects(j).x,cameraObjects(j).y,"o",'MarkerSize',8,'LineStyle',"-.",'MarkerEdgeColor',[0 0 0.2]);
...
for j=1:obstacle_const.max_obs_camera
    if MDCUObjects(j).flag
        cntMDCU = cntMDCU+1;
        x_MDCU(cntMDCU) = MDCUObjects(j).x;
        y_MDCU(cntMDCU) = MDCUObjects(j).y;
        vx_MDCU(cntMDCU) = MDCUObjects(j).velo_x;
        vy_MDCU(cntMDCU) = MDCUObjects(j).velo_y;
%         plot(cameraObjects(j).x,cameraObjects(j).y,"o",'MarkerSize',8,'LineStyle',"-.",'MarkerEdgeColor',[0 0 0.2]);
        %             cl(cnt) = obs_XYs(j).cls+uint8(1);
    end
end
...
% vx(:, vcnt+1:end) = [];
% vy(:, vcnt+1:end) = [];
x_MDCU(cntMDCU+1:end) = [];
y_MDCU(cntMDCU+1:end) = [];
% 跟踪库中的有效目标
% if ecnt > 0 
...
% 单帧MDCU数据
if cntMDCU >0
    scatter(ax,x_MDCU,y_MDCU,'pentagram','MarkerEdgeColor',[0 0 0.2]);
    line(vx_MDCU, vy_MDCU, 'color', 'blue', 'linewidth', 1)
end
